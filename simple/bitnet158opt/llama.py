...
from bitnet158opt import BitLinear158Opt
import torch
...
class BitLlamaMLP(LlamaMLP):
    def __init__(self, config):
        ...
        self.gate_proj = BitLinear158Opt(self.hidden_size, self.intermediate_size, bias=False)
        self.up_proj = BitLinear158Opt(self.hidden_size, self.intermediate_size, bias=False)
        self.down_proj = BitLinear158Opt(self.intermediate_size, self.hidden_size, bias=False)


class BitLlamaAttention(LlamaAttention):
    def __init__(self, config: BitLlamaConfig, layer_idx: Optional[int] = None):
        super().__init__(config, layer_idx)
        self.q_proj = BitLinear158Opt(self.hidden_size, self.num_heads * self.head_dim, bias=False)
        self.k_proj = BitLinear158Opt(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=False)
        self.v_proj = BitLinear158Opt(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=False)
        self.o_proj = BitLinear158Opt(self.hidden_size, self.hidden_size, bias=False)
...
class BitLlamaForCausalLM(LlamaForCausalLM):
    ...
    def __init__(self, config: BitLlamaConfig):
        ...
        self.lm_head = BitLinear158Opt(config.hidden_size, config.vocab_size, bias=False)
