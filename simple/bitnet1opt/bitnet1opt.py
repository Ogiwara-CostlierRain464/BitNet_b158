...
class BitLinear1Opt(nn.Linear):
    ...
    def __init__(self, in_features, out_features, bias=False):
        super(BitLinear1Opt, self).__init__(in_features, out_features, bias)
        self.layer_norm = BitRMSNorm(hidden_size=in_features, eps=self.eps)
        # self.layer_norm = LN() # BitNet1の元論文

    def activation_quant(self, x: torch.Tensor, before_linear=True) -> tuple[Tensor, float]:
        eps = self.EPS
        Qb = self.Qb
        if before_linear:
            # (4), (5)
            ...
            eta = x.min()
            gamma: float = (x - eta).abs().max(dim=-1, keepdim=True).values.clamp_(min=eps)
            x_scaled = (x - eta) * Qb / gamma
            x_q = ((x - eta) * Qb / gamma).round().clamp(0, Qb - 1)
